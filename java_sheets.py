...
# Opening the sheet
sheet = client.open(
    "Indorse Java Validator | Special Assignment (Responses)").sheet1
...
def updating_google_sheet():
    ...
    for i in range(len(github_user_list)):
        if github_user_list[i] == "":
            ...
            r = r.json()
            r_java = [dict_result for dict_result in r if dict_result['language'] == 'Java']
            sheet.update_cell(user_cell_range[i], 12, len(r_java))
    print("Java File: All the Github repos for the pending rows have been identified and necessary data has been inserted")


if last_read_row == total_rows:
    print("Java File: You have already read and updated all the rows")
else:
